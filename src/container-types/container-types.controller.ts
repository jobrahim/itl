import { Controller, Post, Body, Res, UseGuards, Put, Param, Delete } from '@nestjs/common';
...
export class ContainerTypesController {
 ...
 // @UseGuards(JwtAuthGuard)
  ...
  @Put(':id')
  update(@Param('id') id: string, @Body() updateContainerTypeDto: UpdateContainerTypeDto, @Res() response) {
    this.containerTypesService.update(+id, updateContainerTypeDto).then(updateContainer => {
  ...
  remove(@Param('id') id: string, @Res() response) {
    this.containerTypesService.remove(+id).then(removeRange => {
      return response.status(HttpStatus.OK).json({ "success": true, "message": "" })
    }).catch(() => {
      return response.status(HttpStatus.FORBIDDEN).json({ error: "error" });
    });
